...
if($prefArrayName=="gp_socialmediaTable") $suid_PageAccess = true;

if(!$PrevPage){
	$JumpToThisPage = "admin_genericUpdate.php";
}else{
...
if( notloggedin() ) {
...
}else if($_SESSION['suid'] && $suid_PageAccess && !suid_pageAccess($itemID)){
	...
	$field_itemID = $prefArray['itemID']['field'];
	$WHERE = "WHERE $field_id=$editid";
	...
	if($tablename){
		...
		if($adding && $_POST['adding']==1){
			...
			for ($i = 0; $i < sizeof($field_divs); $i++) {
				...
				if(isset($_POST[$field_divs[$i]])){
					...
					$update_query = "UPDATE $FULLtablename SET ${field_divs[$i]}='${_POST[$field_divs[$i]]}' $WHERE LIMIT 1";
			...
			if($editid){
				$value_query = "SELECT * FROM $FULLtablename $WHERE LIMIT 1";				
		...
		if($PrintForm){
			$buildForm = '';
			$buildForm .= '<form enctype="multipart/form-data" method="post" action="admin_genericUpdate.php" name="GenericForm">';
			...
			$buildForm .= '<input type="hidden" name="itemID" value="'.$itemID.'">';
						...
						if($prefArray[$fname]['SelectFromDB']){							
							...
							switch($ftype){
								...
								case "datetime":	
												...
												if($ftype=="datetime"){
													$tmpValue = $defaultDate['datetime'];
													$pShowTime='true';
												}else{
													$tmpValue = $defaultDate['date'];
													$pShowTime='false';
												}
												if(!empty(${"value_".$i}) && ${"value_".$i} != $defaultDate['date'] && ${"value_".$i} != $defaultDate['datetime']){
													$tmpValue = ${"value_".$i};																									
												}
												$buildForm .= 'value="'.$tmpValue.'"';
			...
			if($editid || $addedid){
				...
				}elseif($prefArrayName=="gp_socialmediaTable"){					
					$SubNavInner .= '<li class="edit"><a href="admin_catalogue_upload.php?editid='.$itemID.'" title="Delete">return to edit profile</a></li>';
				}else{
	...
	echo '<input type="submit" id="submit" name="Submit" value="Update Details">';
	echo '</form>';
	*/
}
